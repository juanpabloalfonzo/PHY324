...
# plt.title('Ratio Points Inside Circle vs Number of Points')
...
#         X[i]=x #Storing Position 
#         Y[i]=y 
#     num_inside1[z]=np.mean(X**2+Y**2) #Storing average of X**2+Y**2 values inside circle 


# mean1=np.mean(num_inside1)
# un1=np.std(num_inside1)   
...
#         X[i]=x #Storing Position 
#         Y[i]=y
#     num_inside2[z]=np.mean(X**2+Y**2) #Storing average of X**2+Y**2 values inside circle 

# mean2=np.mean(num_inside2)
# un2=np.std(num_inside2)  
...
for z in range(100): #Runs the trial 100 times
    ...
    for i in range(100000): 
        ...
        Y[i]=y
    inside=np.where(X**2+Y**2<1) #Finding points inside circle
    num_inside3[z]=np.mean(X[inside]**2+Y[inside]**2) #Storing average of X**2+Y**2 values inside circle 

mean3=np.mean(num_inside3)
un3=np.std(num_inside3)  
